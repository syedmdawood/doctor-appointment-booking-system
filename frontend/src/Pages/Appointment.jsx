...
const Appointment = () => {
  ...
  const getAvailableSlot = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // setting the time and dat with index
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:4000/api/user/book-appointment/${docId}`,
      ...
      if (data.success) {
        ...
        toast.error(data.message); // This will show the error message from backend
      }
    } catch (error) {
      console.log(error);
      // Improved error handling to provide a clearer message to the user
      toast.error(
